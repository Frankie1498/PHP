<?php


class Database
{
    protected $connect;
    protected $database;
    protected $table;
    protected $resultQuery;
    //CONNECT
    public function __construct($params)
    {
       $link = mysqli_connect($params['sever'],$params['user'],$params['password']);
       if (!$link) die('Fail Connect');
       else {
           $this->connect = $link;
           $this->database = $params['database'];
           $this->table = $params['table'];
           $this->setDatabase();
           $this->query("SET NAMES 'utf8'");
           $this->query("SET CHARACTER 'utf8'");
       }
    }
    //SET CONNECT
    public function setConnect($connect){
        $this->connect = $connect;
    }
    //SET DATABASE
    public  function setDatabase($database = null){
        if ($database != null) $this->database = $database;
        mysqli_select_db($this->connect,$this->database);
    }
    //SET TABLE
    public function setTable($table){
        $this->table = $table;
    }
    //CREATE INSERT SQL
    public function createInsertSQL($data){
        $newQuery = array();
        if (!empty($data)){
            $cols= '';
            $vals= '';
            foreach ($data as $key => $value) {
                $cols .= ",`$key`";//`name`, `status`, `ordering`
                $vals .= ",'$value'";
            }
        }
        $newQuery['cols'] = substr($cols,1);
        $newQuery['vals'] = substr($vals,1);
        return $newQuery;
    }
    //INSERT
    public  function insert($data,$type = 'single'){
        if ($type == 'single'){
            $newQuery = $this->createInsertSQL($data);
            $query    = "INSERT INTO `".$this->table."`(".$newQuery['cols'].") VALUES (".$newQuery['vals'].")";
            $this->query($query);
        }else{
            //echo 'hi';
            foreach ($data as $value){
                $newQuery = $this->createInsertSQL($value);
                /*echo '<pre>';
                print_r($newQuery);
                echo '</pre>';*/
                $query    = "INSERT INTO `".$this->table."`(".$newQuery['cols'].") VALUES (".$newQuery['vals'].")";
                $this->query($query);
            }
        }
        return $this->lastID();
    }
    //CREATE UPDATE SQL
    function createUpdateSQL($data){
        $newQuery = '';
        if (!empty($data)){
            foreach ($data as $key => $value){
                $newQuery .= ",`$key` = '$value'";
            }
            $newQuery = substr($newQuery,1);

        }
        return $newQuery;
    }
    //CREATE WHERE SQL
    function createWhereUpdateSQL($where){

        $newWhere = array();
        if (!empty($where)){
            foreach ($where as $value){
                $newWhere[] = "`$value[0]` = '$value[1]'";
                if (isset($value[2])) $newWhere[] = $value[2];
            }
            $newWhere = implode(" ",$newWhere);
        }
        return $newWhere;
    }
    //UPDATE
    public function update($data,$where){
        /*echo '<pre>';
        print_r($data);
        echo '</pre>';*/
        $newSet = $this->createUpdateSQL($data);
        $newWhere = $this->createUpdateSQL($where);
        $query    = "UPDATE `".$this->table."` SET ".$newSet." WHERE ".$newWhere."";
        $this->query($query);
        return $this->affectedRow();
    }
    //
    public function createWhereDeleteSQL($data){
        $newWhere = '';
        if (!empty($data)){
            foreach ($data as $value){
                $newWhere .= ','."'$value'";
            }
            $newWhere = substr($newWhere,1);
            $newWhere = '('.$newWhere.')';
            /*echo '<pre>';
            print_r($newWhere);
            echo '</pre>';*/
        }
        return $newWhere;
    }
    //DELETE
    public  function delete($data,$where){
        $newWhere = $this->createWhereDeleteSQL($where);
        $query = "DELETE FROM `".$this->table."`WHERE `".$data."` IN ".$newWhere." ";
        $this->query($query);

       // echo $query;
        return $this->affectedRow();
    }
    //LIST RECORD
    public function listRecord($resultQuery = null){
        $result = array();
        $resultQuery = ($resultQuery == null)? $this->resultQuery : $resultQuery;
        if (mysqli_num_rows($resultQuery)>0){
            while ($row = mysqli_fetch_assoc($resultQuery)){
                $result[] = $row;
            }
            mysqli_free_result($resultQuery);
        }
        return $result;
    }
    //SINGLE RECORD
    public function singleRecord($resultQuery = null){
        $result = array();
        $resultQuery = ($resultQuery == null)? $this->resultQuery : $resultQuery;
        if (mysqli_num_rows($resultQuery)>0){
                $result[] = mysqli_fetch_assoc($resultQuery);
        }
        mysqli_free_result($resultQuery);
        return $result;
    }
    //TOTAL ITEM
    public function totalItems($query){

        if (!empty($query)){
            $resultQuery = $this->query($query);
            if (mysqli_num_rows($resultQuery)>0) $result = mysqli_fetch_assoc($resultQuery);
        }
        return $result['totalItems'];
    }
    //LAST ID
    public  function lastID(){
        return mysqli_insert_id($this->connect);
    }
    //QUERY
    public function query($query){
        $this->resultQuery = mysqli_query($this->connect,$query);
        return $this->resultQuery;
    }
    //AFFECTED ROW
    public function affectedRow(){
        return mysqli_affected_rows($this->connect);
    }


    //DISCONNECT
    public function __destruct()
    {
        mysqli_close($this->connect);
    }

}